...
%token ARRAY BEGIN SYS_TYPE CONST END FUNCTION PROGRAM
%token PROCEDURE RECORD VAR TYPE 
%token EQUAL UNEQUAL GE GT LE LT AND OR NOT 
%token PLUS MINUS MUL DIV DIVI MOD DOTDOT
%token IDENTIFIER INTEGER REAL CHAR STRING SYS_CON 
%token READ SYS_PROC SYS_FUNCT 
%token DOWNTO DO REPEAT TO THEN WHILE UNTIL FOR
%token IF ELSE CASE OF GOTO 

%type<Sval> IDENTIFIER SYS_CON SYS_FUNCT SYS_PROC SYS_TYPE READ
%type<Ival> INTEGER
%type<Rval> REAL
%type<Cval> CHAR
...
const_part : 
    ...
    |                                                                { $$ = new ConstDeclList(); }
                                                                     ;
const_expr_list : 
    const_expr_list  ID  EQUAL  const_value  SEMI                  { $$ = $1; $$->push_back(new ConstDeclaration($2, $4)); }
    |  ID  EQUAL  const_value  SEMI                                { $$ = new ConstDeclList(); $$->push_back(new ConstDeclaration($1, $3)); }
                                                                     ;
const_value :
...
type_definition : 
    ID  EQUAL  type_decl  SEMI                                     { $$ = new TypeDeclaration($1, $3); }
                                                                     ;
...
simple_type_decl : 
    ...
    |  ID  DOTDOT  ID                                            { $$ = new AstType(new EnumRangeType($1, $3)); }
                                                                     ;
...
function_head :  FUNCTION  ID  parameters  COLON  simple_type_decl { $$ = new FuncDeclaration($2, $3, $5); }
                                                                     ;

procedure_decl :  procedure_head  SEMI  sub_routine  SEMI            { $$ = $1; $$->setRoutine($3); }
                                                                     ;

procedure_head :  PROCEDURE ID parameters                          { $$ = new FuncDeclaration($2, $3); }
                                                                     ;
...
compound_stmt : BEGIN  stmt_list  END                                { $$ = new CompoundStatement($2); }
                                                                     ;
stmt_list : 
...
assign_stmt : 
    ID  ASSIGN  expression                                         { $$ = new AssignStatement($1, $3); }
    | ID LB expression RB ASSIGN expression                        { $$ = new AssignStatement($1, $3, $6); }
    | ID  DOT  ID  ASSIGN  expression                            { $$ = new AssignStatement($1, $3, $5); }
                                                                     ;

proc_stmt : 
    ID                                                             { $$ = new ProcedureCall($1); }
    |  ID  LP  args_list  RP                                       { $$ = new ProcedureCall($1, $3); }
...
for_stmt : 
    FOR  ID  ASSIGN  expression  direction  expression  DO stmt    { $$ = new ForStatement($2, $4, $5, $6, $8); }
                                                                     ;
...
case_expr : 
    ...
    |  ID  COLON  stmt  SEMI                                       { $$ = new CaseExpression($1, $3); }
                                                                     ;
...
factor : 
    ID                                                             { $$ = $1; }
    |  ID  LP  args_list  RP                                       { $$ = new FunctionCall($1, $3); }
    ...
    |  ID  LB  expression  RB                                      { $$ = new ArrayReference($1, $3); }
    |  ID  DOT  ID                                               { $$ = new RecordReference($1, $3); }
;
args_list : 
